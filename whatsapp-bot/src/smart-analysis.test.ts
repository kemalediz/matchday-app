...
// `isSelfMention` MOVED to `mentions.ts` (2026-09-08) and its seven
// regression cases went with it, to `mentions.test.ts`. The body rewrite
// and the `botMentioned` flag are now decided by the same function
// (`contactIsBot`), so there is one rule rather than two that could
// disagree about who the bot is, and the cases now run through
// `rewriteMentions` — the function production calls.
